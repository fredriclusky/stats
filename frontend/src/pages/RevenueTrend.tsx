  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-xl font-bold text-white">Revenue Trend Drilldown</h1>
          <div className="text-gray-500 text-sm">Click a date to view revenue by hour.</div>
        </div>
      ...
      {selectedDate && (
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-4">
          <div className="flex items-center justify-between mb-4">
            <div className="text-gray-400 text-sm font-medium">Hourly Revenue - {selectedDate}</div>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={hourly}>
              ...
              <YAxis tick={{ fontSize: 11, fill: '#6b7280' }} tickLine={false} axisLine={false} tickFormatter={(v) => '$' + Number(v).toFixed(0)} />
              <Tooltip
                contentStyle={{ background: '#111827', border: '1px solid #374151', borderRadius: 8 }}
                formatter={(v: any) => [fmt(Number(v)), 'Revenue']}
              />
              <Area type="monotone" dataKey="revenue" stroke="#22c55e" strokeWidth={2} fill="url(#hourlyRevenue)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      )}
